...
var ReloadProjectsCommand = class {
  ...
  async execute() {
    ...
    import_coc2.window.showInformationMessage("projects reloaded");
  }
};
...
async function goToProjectConfig(clientHost, uri) {
  ...
  if (!res || !res.body) {
    import_coc2.window.showWarningMessage("Could not determine TypeScript or JavaScript project.");
    return;
  }
  ...
  import_coc2.window.showWarningMessage("Config file not found");
}
...
var _FileReferencesCommand = class {
  ...
  async execute() {
    ...
    if (client.serviceClient.apiVersion.lt(_FileReferencesCommand.minVersion)) {
      import_coc2.window.showErrorMessage("Find file references failed. Requires TypeScript 4.2+.");
      return;
    }
...
var _SourceDefinitionCommand = class {
  ...
  async execute() {
    ...
    if (client.serviceClient.apiVersion.lt(_SourceDefinitionCommand.minVersion)) {
      import_coc2.window.showErrorMessage("Go to Source Definition failed. Requires TypeScript 4.7+.");
      return;
    }
    const { document, position } = await import_coc2.workspace.getCurrentState();
    if (client.serviceClient.modeIds.indexOf(document.languageId) == -1) {
      import_coc2.window.showErrorMessage("Go to Source Definition failed. Unsupported file type.");
      return;
    }
    const openedFiledPath = client.serviceClient.toOpenedFilePath(document.uri);
    if (!openedFiledPath) {
      import_coc2.window.showErrorMessage("Go to Source Definition failed. Unknown file type.");
      return;
    }
    await import_coc2.window.withProgress({ title: "Finding source definitions" }, async (_progress, token) => {
      ...
      if (response.type === "response" && response.body) {
        ...
        if (locations.length) {
          import_coc2.commands.executeCommand("editor.action.showReferences", document.uri, position, locations);
          return;
        }
      }
      import_coc2.window.showErrorMessage("No source definitions found.");
    });
  }
};
...
var _WatchProject = class {
  ...
  async getOptions() {
    ...
    if (!tscPath) {
      import_coc4.window.showErrorMessage(`Local & global tsc not found`);
      return;
    }
    let doc = await import_coc4.workspace.document;
    const tsconfigPath = import_coc4.workspace.getConfiguration("tsserver", doc.uri).get("tsconfigPath", "tsconfig.json");
    let find = await import_coc4.workspace.findUp([tsconfigPath]);
    if (!find) {
      import_coc4.window.showErrorMessage(`${tsconfigPath} not found!`);
      return;
    }
...
var TypeScriptWorkspaceSymbolProvider = class {
  ...
  async provideWorkspaceSymbols(search, token) {
    ...
    for (const item of response.body) {
      ...
      const symbolInfo = import_vscode_languageserver_protocol3.SymbolInformation.create(
        label,
        getSymbolKind(item),
        range,
...
var TypeScriptCompletionItemProvider = class {
  ...
  async resolveCompletionItem(item, token) {
    ...
    if (!item.data.isSnippet && detail && item.insertTextFormat == import_vscode_languageserver_protocol9.InsertTextFormat.Snippet) {
  ...
  isInValidCommitCharacterContext(document, position) {
    if (this.client.apiVersion.lt(API.v320)) {
      if (position.character > 1) {
        const preText = document.getText(import_vscode_languageserver_protocol9.Range.create(
          position.line,
          0,
          position.line,
          position.character
...
var TypeScriptDocumentSymbolProvider = class {
  ...
  static convertSymbol(item, range) {
    ...
    const symbolInfo = import_vscode_languageserver_protocol11.DocumentSymbol.create(
      label,
      "",
...
var _ApplyRefactoringCommand = class {
  ...
  async execute(document, file2, refactor, action, range) {
    ...
    if (renameLocation) {
      import_coc20.commands.executeCommand(
...
var _SelectRefactorCommand = class {
  ...
  async execute(document, file2, info, range) {
    let { actions } = info;
    const idx = actions.length == 1 ? 0 : await import_coc20.window.showQuickpick(
...
var _TypeScriptRefactorProvider = class {
  ...
  async provideCodeActions(document, range, context, token) {
    ...
    const args = Range3.toFileRangeRequestArgs(
      file2,
      range
    );
    let response;
    ...
    return this.convertApplicableRefactors(
      ...
      file2,
      range,
      context.only && context.only.some((v) => v.includes(import_vscode_languageserver_protocol15.CodeActionKind.Refactor))
    );
  }
...
var _TagClosing = class {
  ...
  async onInsertChange(bufnr, change, lastInsert) {
    ...
    this._timeout = setTimeout(async () => {
      ...
      if (doc.changedtick === changedtick) {
        import_coc24.snippetManager.insertSnippet(
          this.getTagSnippet(insertion).value,
          false,
          import_coc24.Range.create(position, position)
        );
      }
    }, 30);
  }
...
var UpdateImportsOnFileRenameHandler = class {
  ...
  async doRename(oldResource, newResource) {
    ...
    if (!document) {
      ...
      import_coc25.workspace.nvim.command("checktime", true);
    }
    if (!document)
      return;
    await wait(50);
...
var LanguageProvider = class {
  ...
  registerProviders(client, typingsStatus) {
    ...
    this._register(
      import_coc27.languages.registerCompletionItemProvider(
        clientId,
        "TSC",
        languageIds,
    ...
    if (this.client.apiVersion.gte(API.v230)) {
      this._register(import_coc27.languages.registerCompletionItemProvider(
    ...
    if (this.client.apiVersion.gte(API.v280)) {
      ...
      this._register(
        import_coc27.languages.registerCodeActionProvider(
          languageIds,
    ...
    if (this.client.apiVersion.gte(API.v240)) {
      this._register(
        import_coc27.languages.registerCodeActionProvider(
          ...
          "tsserver",
          [import_vscode_languageserver_protocol17.CodeActionKind.Refactor]
        )
      );
    }
    this._register(
      import_coc27.languages.registerCodeActionProvider(
        languageIds,
        new TypeScriptQuickFixProvider(client, this.fileConfigurationManager),
        "tsserver",
        [import_vscode_languageserver_protocol17.CodeActionKind.QuickFix]
      )
    );
    this._register(
      import_coc27.languages.registerCodeActionProvider(
...
var BufferSyncSupport = class {
  ...
  listen() {
    if (this.listening)
      return;
    this.listening = true;
    import_coc29.workspace.onDidOpenTextDocument(
      this.openTextDocument,
      this,
      this.disposables
    );
    import_coc29.workspace.onDidCloseTextDocument(
      this.onDidCloseTextDocument,
      this,
      this.disposables
    );
    import_coc29.workspace.onDidChangeTextDocument(
...
var TypeScriptServiceConfiguration = class {
  constructor() {
    ...
    import_coc31.workspace.onDidChangeWorkspaceFolders((e) => {
      if (e.added.length > 0) {
        this._configuration = import_coc31.workspace.getConfiguration("tsserver");
        this._includePackageJsonAutoImports = import_coc31.workspace.getConfiguration("typescript").get("preferences.includePackageJsonAutoImports");
      }
...
var Tracer = class {
  ...
  traceResponse(response, startTime) {
    ...
    this.logTrace(
      `Response received: ${response.command} (${response.request_seq}). Request took ${Date.now() - startTime} ms. Success: ${response.success} ${!response.success ? ". Message: " + response.message : ""}`,
      data
    );
  }
  traceEvent(event) {
...
var TypeScriptVersionProvider = class {
  ...
  get globalVersion() {
    let { globalTsdk } = this.configuration;
    let folder = import_coc34.workspace.expand(globalTsdk);
    if (!import_path7.default.isAbsolute(folder)) {
      folder = import_path7.default.join(import_coc34.workspace.root, folder);
    }
    if (globalTsdk)
  ...
  get bundledVersion() {
    try {
      ...
      const bundledVersion = new TypeScriptVersion(
        import_path7.default.dirname(file2),
        ""
      );
      return bundledVersion;
    } catch (e) {
      import_coc34.window.showErrorMessage("Bundled typescript module not found");
      return null;
    }
  }
};
...
var TypeScriptServiceClient = class {
  constructor(pluginManager, modeIds, tscPathVim) {
    ...
    this.emptyAuthority = "ts-nul-authority";
  ...
  startService(resendModels = false) {
    ...
    if (!currentVersion || !import_fs4.default.existsSync(currentVersion.tsServerPath)) {
      ...
      if (currentVersion)
        this.info("Local and global tsserver not found, using global tsserver from configuration");
    }
    if (!currentVersion || !import_fs4.default.existsSync(currentVersion.tsServerPath)) {
  ...
  startProcess(currentVersion, args, options, resendModels) {
    ...
    try {
      ...
      handle.onError((err) => {
        ...
        import_coc37.window.showErrorMessage(`TSServer errored with error. ${err.message}`);
    ...
    if (restart) {
      ...
      if (this.numberRestarts > 5) {
        ...
        if (diff < 10 * 1e3) {
          this.lastStart = Date.now();
          startService = false;
          import_coc37.window.showErrorMessage("The TypeScript language service died 5 times right after it got started.");
        } else if (diff < 60 * 1e3) {
          this.lastStart = Date.now();
          import_coc37.window.showErrorMessage("The TypeScript language service died unexpectedly 5 times in the last 5 Minutes.");
        }
      }
      if (startService) {
        this.startService(true);
        ...
        if (this.inMemoryResourcePrefix) {
          ...
          if (fileName.startsWith(this.inMemoryResourcePrefix)) {
            resource = resource.with({
              path: import_path8.default.posix.join(
                dirName,
  ...
  async onTypesInstallerInitializationFailed() {
    ...
    if (!this._invalid) {
      ...
      if (config.get("check.npmIsInstalled", true)) {
        ...
        const selected = await import_coc38.window.showWarningMessage(
